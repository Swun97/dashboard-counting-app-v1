...
sudo curl -LO https://github.com/hashicorp/demo-consul-101/releases/download/v0.0.5/dashboard-service_linux_amd64.zip
sudo unzip dashboard-service_linux_amd64.zip
sudo rm -rf dashboard-service_linux_amd64.zip
sudo mv dashboard-service_linux_amd64 dashboard-service
sudo mv dashboard-service /usr/bin/dashboard-service
sudo chmod 755 dashboard-service
...
Environment=COUNTING_SERVICE_URL="http://${counting_private_ip}:8888"
